## UNIT-1
1. Explain the following terms: *i) System, ii) Surrounding, iii) Control volume, iv) Universe and v) Types of System.*
2. Explain about the concept of continuum with diagram. How will you correlate with density and pressure using this concept?
3. Define the following terms: *i) State, ii) Property, iii) Process iv) Cycle.*
4. Explain about Thermodynamic Equilibrium, its types along with suitable examples.
5. Identify whether the system is open or closed: *i) A tube of a bicycle filled with air, ii) A jet engine in flight, iii) A household refrigerator in operation, iv) Water pump, v) Pressure cooker without whistling, vi) Steam turbine vii) Car battery, viii) An electric geyser.*
6. Explain quasi-static process. State its salient features.
7. Distinguish between macroscopic and microscopic views.
8. Explain reversible and irreversible processes, and causes of irreversibly.
9. Explain Zeroth law of thermodynamics and concept of temperature.
10. Explain work and heat, and its types and sign conventions.


## UNIT-2

1 State and explain the First Law of Thermodynamics for a closed 
system undergoing a cyclic process.
2 Define the following terms i) Joule’s Experiment, ii) Energy in 
State and iii) Energy in Transition. L2 CO2 8M
3 Explain the following terms i) Point function, ii) Path function, iii) 
Enthalpy, and iv) Specific heat at constant pressure. L2 CO2 8M
4 a Explain about PMM-I. Why such a machine is considered 
impossible according to the First Law of Thermodynamics, 
explain with details.
L2 CO2 4M
b During a certain process, the specific heat capacity of a system is 
given by C = (0.4 + 0.004 T) kJ/kg°C. Find the heat transferred 
and mean specific heat of gas, when the temperature changes from 
25°C to 125°C. The mass of the system is 5 kg.
L3 CO2 4M
5 Derive the steady-flow energy equation along with its 
assumptions. L3 CO2 8M
6 Discuss the applications of steady-flow energy equation in devices 
such as nozzles, diffusers, turbines, compressors, and throttling 
along with its assumptions.
L3 CO2 8M
7 a A fluid at a pressure of 3 bar, and with specific volume of 0.18 
m3
/kg, contained in a cylinder behind a piston expands reversibly 
to a pressure of 0.6 bar according to a law, p = Cv2 where C is a 
constant. Calculate the work done by the fluid on the piston.
L3 CO2 4M
b Discuss the limitations of the First Law of Thermodynamics. L2 CO2 4M
8 Air flows steadily at the rate of 0.5 kg/s through an air 
compressor, entering at 7m/s velocity, 100 kPa pressure, and 0.95 
m3
/kg volume, and leaving at 5 m/s, 700 kPa, and 0.19 m3
/kg. The 
internal energy of the air leaving is 90 kJ/kg greater than that of 
the air entering. Cooling water in the compressor jackets absorbs 
heat from the air at the rate of 58 kW. 
(a) Compute the rate of shaft work input to the air in kW. 
(b) Find the ratio of the inlet pipe diameter to outlet pipe 
diameter.
